...
func TestBuildV2RejectsCurrentStoreSkill(t *testing.T) {
	src := &imagefile.V2{
		SchemaVersion: 2,
		Dir:           t.TempDir(),
		Skills:        []imagefile.SkillEntry{{Dir: "$CURRENT_VERSION_STORE/skills/whoami"}},
	}
	if _, err := BuildV2(src, imagefile.ResolveRoots{}, Ref{Name: "versioned", Tag: "latest"}, &Store{Dir: t.TempDir()}, time.Now, nil); err == nil {
		t.Fatal("$CURRENT_VERSION_STORE skill was accepted")
	}
}
