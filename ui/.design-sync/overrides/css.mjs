// forked from design-sync lib/css.mjs - annotate non-token custom properties in
// _ds_bundle.css with `/* @kind other */` before styleSha is computed.
//
// Tariboy ships no static stylesheet, so _ds_bundle.css is the ONLY place its
// CSS lives: real theme tokens under :root/.dark sit next to the compiled
// Tailwind utility output, flexlayout-react's generically-named theme vars and
// terminalWorkspace's --splitter-*. The design-system token scan reads all of
// them as tokens. Annotating cfg.cssEntry alone is not enough - the converter
// appends ~48KB of CSS that esbuild pulls out of the JS bundle (flexlayout,
// xterm), which never passes through cssEntry.
//
// writeStylesCss is the seam: package-build.mjs calls it after _ds_bundle.css
// is final and before styleShaFor() runs, so the anchor describes the CSS that
// actually ships. The pass is metadata only - nothing is removed, and the CSS
// is byte-identical once comments are stripped.
...
import { annotateFile } from '../annotate-tokens.mjs';
...
export function writeStylesCss({ out, tokenFiles, bundleCss, fontRules, remoteImports }) {
  // FORK: mark everything in _ds_bundle.css that is not a design token.
  annotateFile(bundleCss ?? join(out, '_ds_bundle.css'));
