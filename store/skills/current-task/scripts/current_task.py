...
if __name__ == "__main__":
    args = sys.argv[1:]
    if args in (["-h"], ["--help"]):
        print("usage: current_task.sh <task-key|--clear> [--json]")
        raise SystemExit(0)
    if "--json" in args:
        os.environ["TARIBOY_TOOLS_JSON"] = "1"
    ...
    if not args:
        ...
        raise SystemExit(2)
    if any(arg.startswith("--clear=") for arg in args):
        print("tools task current: --clear does not take a value", file=sys.stderr)
        raise SystemExit(2)
    if len(args) != 1:
        print("tools task current: pass exactly one <task-key> or --clear", file=sys.stderr)
        raise SystemExit(2)
    raise SystemExit(main("POST", "/tools/task/current", {"id": args[0]}, cli_args=args))
