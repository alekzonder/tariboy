...
def call(method, route, body=None):
    ...
    if daemon_version and daemon_version != version:
        print(f"warning: client version {version} does not match daemon version {daemon_version}; this client ({sys.argv[0]}) may not know the daemon's newer flags", file=sys.stderr)
