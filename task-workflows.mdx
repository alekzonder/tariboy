...
| `outcomes` | list of strings | Yes | One or more unique, non-empty values accepted by `ttasks work complete`. Guards may only name outcomes declared here. |
...
`assignment`, or `requirement`) through `ttasks ask`.
...
An assignment can subscribe only with `ttasks observe subscribe` while its lease
...
the semantic result. Managed tasks are excluded from legacy `ttasks ready
--claim` and cannot be driven by direct task status/claim mutations.

## Work packets and artifacts

`ttasks work next` atomically claims one eligible assignment and returns the
...
```bash
ttasks work next --queue DEV --idempotency-key iter-42-claim
ttasks work show 17
ttasks artifacts add 17 --name implementation --type commit \
  --content abc123 --task-revision 3 --assignment-revision 2 \
  --idempotency-key iter-42-implementation
ttasks artifacts add 17 --name tests --type markdown \
  --content "go test ./...: PASS" --task-revision 4 --assignment-revision 3 \
  --idempotency-key iter-42-tests
ttasks work complete 17 --outcome implemented \
  --task-revision 5 --assignment-revision 4 \
  --idempotency-key iter-42-complete
```

Use `ttasks work release 17` with both revisions and an idempotency key when the
agent cannot finish this attempt. `ttasks artifacts show 17 9 --task DEV-4`
...
manager claims the question through `ttasks work next`, inspects it with
`ttasks questions ASSIGNMENT`, and answers:

```bash
ttasks answer 6 --assignment 21 --answer "Reject empty owners." \
...
path. This mechanism is separate from legacy `ttasks ask KEY principal TEXT`,
...
```bash
ttasks observe subscribe 17 logs:dev --reaction wake_current \
  --task-revision 5 --assignment-revision 4 \
  --idempotency-key iter-42-observe
ttasks observe list 17
ttasks observe cancel 17 4 --task-revision 6 --assignment-revision 5 \
...
`ttasks` uses identity-bound agent mode only when `TARIBOY_TOOLS_SOCKET` is
non-empty and fails closed if that socket cannot be used. Without it, the
client uses the host Unix daemon socket as the customer actor. The bare `tasks`
shim remains only as a legacy capability-controlled compatibility command.
Workflow operations are:

```text
ttasks work next [--queue Q] --idempotency-key KEY
ttasks work show ASSIGNMENT
ttasks work complete ASSIGNMENT --outcome O --task-revision N --assignment-revision N --idempotency-key KEY
ttasks work release ASSIGNMENT --task-revision N --assignment-revision N --idempotency-key KEY
ttasks artifacts add ASSIGNMENT --name N --type T [--content V] [--metadata JSON] --task-revision N --assignment-revision N --idempotency-key KEY
ttasks artifacts show ASSIGNMENT ARTIFACT --task KEY
ttasks ask ASSIGNMENT --question Q --context C --blocking-scope none|assignment|requirement [...]
ttasks questions ASSIGNMENT
ttasks answer QUESTION --assignment ASSIGNMENT --answer TEXT [...]
ttasks observe subscribe ASSIGNMENT PATTERN [--correlation-key K] [--reaction R] [...]
ttasks observe list ASSIGNMENT
ttasks observe cancel ASSIGNMENT SUBSCRIPTION [...]
```
...
subscriptions must go through `ttasks observe ...` and match the resolved
...
- `assignment_already_claimed`, `assignment_not_owned`, `lease_expired`: discard
  stale local work and call `ttasks work next` again.
