...
3. Collect independent human labels and rationales without exposing Judge outputs. Preserve unknown or invalid reference labels outside the accuracy denominator; record an uncertain Judge verdict separately as an outcome. With no labeled cases, accuracy is undefined (`0/0`); `0/total` is coverage only.
