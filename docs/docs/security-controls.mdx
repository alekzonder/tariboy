...
`0.57.1` is ad-hoc signed, not Developer ID signed or notarized. Verify
...
desktop-mac`, and publishes only the DMG, `SHA256SUMS`, and `release.json` with
the job-scoped `contents: write` permission. GitHub publication does not change
