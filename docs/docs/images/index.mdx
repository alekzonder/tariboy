...
| `goal` | Daemon-selected Native Task key, title, priority, status, and description | `tasks` |
...
Each non-empty skill-owned runtime value is preceded by a short instruction to
use that skill; the instruction names the skill without embedding an installation
path. Context data starts with `# Agent Context`, and messages plus outstanding
replies form one `# Messages` group. New templates declare only `messages`;
existing templates that also declare `awaiting-replies` keep working without a
second section. Each singleton placeholder may appear at most once. Empty runtime
...
- `basic:latest` explicitly declares every plugin, packaged Store skill,
  ...
  instruction-only `workdir` plugin, its skill, `runtime: workdir`, and
...
command and instructions. Add `runtime: goal` when it should receive the
daemon-authoritative selected goal; an image without that marker receives no
goal text. The rendered task title and description are untrusted task input,
not daemon instructions or lifecycle authority.
