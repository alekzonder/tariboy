...
while image bytes, source provenance, and build time are shared. Duplicate tags
are rejected. A single tag and an omitted tag remain supported; omission means
`latest`.
