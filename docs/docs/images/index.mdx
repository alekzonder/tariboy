...
| `messages` | Messages delivered to this iteration | `messages` |
| `awaiting-replies` | Outstanding request state | `messages` |
...
path. Context data also starts with `# Agent Context`. Each singleton placeholder
may appear at most once. Empty runtime values add no text. The **Template** tab
shows static paths, categories, sizes, hashes, and runtime markers in their exact
order without draining messages.
...
- `basic:latest` explicitly declares every plugin, packaged Store skill,
  ...
  instruction-only `workdir` plugin, its skill, and `runtime: workdir`. New agents receive the
