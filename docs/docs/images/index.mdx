...
rejected. When the operator CLI omits `--tag`, the build uses `image_version`
from the frozen source. Older files without a version fall back to `latest`.
Explicit tags, including `--tag latest`, take priority.
...
These user Stores are separate from the built-in `store/versions/` tree and
the `$STORE` prompt root.
...
there is no saved image list to become stale. A missing `images/` directory
shows an empty list. An invalid image reports its own error while valid sibling
...
Store builds use the existing image builder. The default result is
`<image_name>:<image_version>`, or `latest` when the source has no version.
Use `--name` to avoid a name collision between Stores, and `--tag` to override
the version. Do not combine a Store selector with `--path`.
...
The Store detail shows **Images**, their versions, and **Build** actions. Built
artifacts appear in the existing **Images** workspace. Stores do not yet expose
standalone skills or plugins.
