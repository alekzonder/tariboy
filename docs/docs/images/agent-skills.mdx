...
`store/images/tariboy-image-creator` creates and improves image sources and
independent Store skills. Configure the agent CWD to the Store root containing
...
managed workdir. Store and plugin references cannot escape their declared
roots. A prompt may use `../` only when its target remains below one of these
explicitly declared source-relative skill directories; the build reads the
file from the same frozen skill snapshot.

For example, `store/images/reviewer/Tariboyfile.yaml` can package
`store/skills/code-review` with:
