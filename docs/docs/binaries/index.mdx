...
This matters because packaged skill scripts and the agent's `tasks` / `i-am-done`
compatibility shims select a versioned Store skill tree: without the warning, a script too old to know a
