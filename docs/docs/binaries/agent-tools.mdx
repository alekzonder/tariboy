...
Goal selection is not an agent command: `tariboyd` selects and persists one
sticky assigned task per enabled agent. An image that declares `runtime: goal`
receives that task as prompt context and should use the `tasks` command to read
