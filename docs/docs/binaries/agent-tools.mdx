...
Goal selection and attribution are not agent commands: `tariboyd` selects and
persists one sticky assigned task per enabled agent, then stamps its key and
top-level root onto the AI-proxy lease before the harness starts. An image that
declares `runtime: goal` receives that task as prompt context and should use
`ttasks` to read the authoritative task state.
