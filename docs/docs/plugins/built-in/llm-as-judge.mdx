...
| Role | Commands |
| --- | --- |
| Lead | `tools judge automation begin`, `iterations search`, `run create`, `run inspect`, `summary claim`, `summary inputs`, `improvement submit`, `summary submit`, `run cancel`, `work retry` |
...
The Store prompt defines the automatic lead/worker discipline and tells workers
