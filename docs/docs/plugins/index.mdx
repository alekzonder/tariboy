...
<CardGroup cols={2}>
  <Card title="Built-in plugins" href="/docs/plugins/built-in" icon="package-check">
    ...
    Tasks, plus instruction-only plugins such as workdir and goal. They do not run as
...
enable a capability; `workdir` and `goal` are instruction-only. Neither class inserts
