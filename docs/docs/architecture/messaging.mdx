...
and counts what that participant has not read; `GET /api/chats/{agent}` returns
that agent's conversation and the customer's current read mark, so a reader can
separate what it has already seen; `POST /api/chats/{agent}/read` moves that mark
forward. Read marks are `chat_participants.read_ts`, one per participant; the
