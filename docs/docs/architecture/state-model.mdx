...
it. One publication gate spans ref-to-digest assignment, automatic activation,
initial provisioning, reprovisioning, controlled rollout staging, import, and
removal, so none can persist an uncommitted generation.
