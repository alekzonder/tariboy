...
stored as a second inventory. This catalog is separate from the built-in
versioned `store/` assets. Store refresh and build preparation are serialized
so a pull cannot race source freezing. Store builds then use the ordinary image
snapshot and publication path below.
