...
`goal_wait_customer_timeout_s` (default 300), and its read-only
`current_goal_task_key`. Disabling Goal clears the selected key; re-enabling it
selects from current task state rather than restoring an old choice. The goal
reconciler is the only writer of the selection.
