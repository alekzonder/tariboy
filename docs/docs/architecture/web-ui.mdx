...
an enabled-by-default switch and a positive whole-second customer-wait timeout
(300 by default). Configuration shows the daemon-selected goal key read-only.
...
revision. When that selected daemon has no `judges` group, the page offers to
create it from the applied document's Judge lead and two workers. Creation and
member assignment use that same explicit daemon target.
