...
inventory, versions and diagnostics, with Refresh, Remove and Build actions.
...
Beside **Send comment**, a secondary **Send Ok** action posts exactly `Ok`
without changing the current comment draft or Ask selection.
...
selected server. A selected server owns Tasks, Images, Stores, and Settings, and its
