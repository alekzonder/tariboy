...
never target the selected local or remote daemon. Automatic download is enabled
by default, persists in WebView localStorage, runs on startup and then every six
hours while enabled, and never installs automatically. A verified package stays
...
remain visible and editable as literal code blocks in rich mode; Source
Markdown stays available as an explicit manual mode.
