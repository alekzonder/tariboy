...
`agent · server`. Choosing an agent on another server saves the other edits,
moves the task with [the server-to-server transfer](/docs/tasks#moving-a-task-to-another-server),
and assigns it on arrival; the sheet then follows the task to its new server.
The transfer's refusals — no queue with that prefix, a taken key, or a
workflow-managed queue — leave the task where it was and are shown as an
error.
