...
An agent's **Messages → Queue** view can mark one row or every currently
pending row processed. The bulk action captures all pending pages before it
starts processing, applies one required result through the existing idempotent
per-message action, and then reloads the queue. Messages published after that
snapshot remain pending.
