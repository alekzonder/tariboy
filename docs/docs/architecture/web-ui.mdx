...
or **All servers**. The UI exports one runnable archive into the open dialog's
browser memory, then previews and applies it against each explicit destination
target. It continues after destination failures; same-digest results are
