...
it selects assigned `in_progress` work before `open`, then orders by priority,
creation time, and task key. Priority changes do not displace a valid selection.
...
loops neither select nor receive a goal wake.
...
inbox message is only a durable wake hint.
