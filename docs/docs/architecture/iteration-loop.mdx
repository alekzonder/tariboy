...
Images with the instruction-only `goal` plugin always render daemon-owned Goal guidance, even without a
...
The built-in `workdir` and `goal` plugins are instruction-only rather than
capabilities. They add no routes or shims; a schema-v2 image explicitly
packages their Store skills and composes the matching runtime value. See
