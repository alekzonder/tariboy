...
supervisor. It then waits as the group's only reaper, using `SIGCHLD` and
`Wait4(WNOHANG)` without polling tmux or starting helper subprocesses.

The supervisor also catches the `SIGHUP` tmux sends when a pane is destroyed.
It checks for an already-exited child first. If the child is still unreaped, the
supervisor force-kills only that owned process group, whose numeric identity is
still pinned by its unreaped leader, and then reaps it. It never discovers a
pane PID from tmux and never signals a process after reaping its leader. The
supervisor does not restore terminal foreground ownership because it performs
no further terminal I/O and exits immediately after recording the result.
