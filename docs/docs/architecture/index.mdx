...
| Agent capability socket | Skill-local scripts plus `tasks` and `i-am-done` compatibility shims | One Unix socket per agent. The daemon derives the agent and current iteration, so request JSON cannot impersonate another caller. |
...
`<data dir>/agents/<name>/bin`: `i-am-done` and `tasks`. Each `exec`s its owning
skill script by an **absolute path** inside the running daemon's versioned
Store. Every other capability is invoked through its packaged skill-local
`scripts/*.sh` launcher. The scripts use
`$TARIBOY_TOOLS_SOCKET`; the daemon derives identity from that socket.
