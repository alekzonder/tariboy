...
one `exec`s Python 3 with an **absolute path** inside the running daemon's
versioned Store: the generic dispatcher under `agent-tools/scripts`, the loop
script for `i-am-done`, or the tasks script. The script then uses
