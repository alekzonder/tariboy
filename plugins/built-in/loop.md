...
| `scripts/loop.sh done` | Socket-facing spelling used by the shim. |
| `scripts/loop.sh start` | Enable and start this agent's loop. |
...
The canonical finishing instructions remain an unconditional prompt tail.
Schema v2 packages the Loop skill and places the mandatory file explicitly:
...
Manager-owned durable `scripts/scripts.sh run` and `scripts/scripts.sh schedule` jobs remain non-blocking because their
